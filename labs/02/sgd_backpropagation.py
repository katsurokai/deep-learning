...
class Model(torch.nn.Module):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # self._b2 = torch.nn.parameter(torch.zeros(MNIST.LABELS)) # Capital letter P is causing module error
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        inputs = torch.tanh(torch.matmul(inputs, self._W1) + self._b1) # hidden layers
        inputs = torch.matmul(inputs, self._W2) + self._b2
        return inputs
    ...
    def train_epoch(self, dataset: MNIST.Dataset) -> None:
        ...
        for batch in dataset.batches(self._args.batch_size):
            ...
            g_label = torch.nn.functional.one_hot(labels.to(torch.int64), num_classes=probabilities.shape[1])
            # log_prob = torch.log(probabilities)
            loss = -torch.log((probabilities * g_label).sum(dim=1)).mean()
    ...
    def evaluate(self, dataset: MNIST.Dataset) -> float:
        ...
        with torch.no_grad():
            ...
            for batch in dataset.batches(self._args.batch_size):
                ...
                labels = batch["labels"].to(self._W1.device)
                logits = self(images)
                # logits_np = logits.numpy(force=True)
